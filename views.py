...
from app import app
from app.modeleML import ClassificationFF2, ClassificationFF4, nerExtraction
...
@app.route('/api/extraction/<path>/', methods=['GET'])
def extraction(path):
    ents = []
    if path != '':
        path = path.replace(os.sep, '/')
        ...
        monRepertoireimg = os.path.join('../' + app.config['UPLOAD_FOLDER'] + '/tmp')
        # shutil.copyfile(path, destination)
        entities = nerExtraction(os.path.join('../' + app.config['UPLOAD_FOLDER']) + '/' + filename,
                                 monRepertoireimg)
        for ent in entities:
